...
document.addEventListener('DOMContentLoaded', async ()=>{
  ...
  await carregaMapa(); carregaLocais(); pedeNome(); addListeners();
});
...
/* ---------- UTIL ---------- */
function atualizaBotao(){btnReg.disabled=!nomeUsuario||enviando;}
function resetForm(){
...
async function enviar(){
  ...
  const payload={usuario:nomeUsuario,codigo,
    nomeProduto:MAPA[codigo]?.nome||'',pesoLiquido:pesoLiq,tara,pesoTotal:pesoTot,
    letraPote:letraPoteSel};

  enviando=true; atualizaBotao();
  ...
  try{
    ...
    if(!r.ok||j.result!=='success')throw new Error(j.message||`HTTP ${r.status}`);
    ...
    console.error('Falha fetch:', e);
    status.textContent='Erro: '+e.message; status.className='error flash-error';
  }finally{
    enviando=false; atualizaBotao(); setTimeout(()=>status.textContent='',7000);
  }
}
